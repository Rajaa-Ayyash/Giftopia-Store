...
export default function RecumentideProduct() {
  ...
  const x = `Giftopia__${token}`
    ...
    useEffect(() => {
      ...
      const getProduct = async () => {
        const response = await axios.get('http://localhost:6060/searchGift/getData',{
            headers : {
                Authorization : x
            }
        ...
        setProduct(response.data.products.product);
      };
  ...
  return (
    ...
    <div className='row'>
        {product.map((produc)=>{
             return <ProductCard img = {produc['ImageURL']}  name = {produc['Title']} price = {parseInt(produc['Price'])} key = {produc._id}  />
        })}
      </div>
    </div>
  )
